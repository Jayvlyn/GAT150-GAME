...
namespace kiko
{
	...
	bool Json::Read(const json_t& value, const std::string& name, class Color& data, bool required)
	{
		// check if 'name' member exists and is an array with 2 elements
	...
	bool Json::Read(const json_t& value, const std::string& name, Rect& data, bool required)
	{
		// check if 'name' member exists and is an array with 2 elements
		if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsArray() || value[name.c_str()].Size() != 4)
		{
	...
	bool Json::Read(const json_t& value, const std::string& name, std::vector<std::string>& data, bool required)
	{
	...
	bool Json::Read(const json_t& value, const std::string& name, std::vector<int>& data, bool required)
	{
		// check if 'name' member exists and is an array 
		if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsArray())
		{
